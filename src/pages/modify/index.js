...
class From extends React.Component {
    ...
    _onKeyDown(e) {
        if (e.keyCode == 9) {
            ...
            target.value = target.value.substring(0, selectionEnd) + "\t" + target.value.substring(selectionEnd)
    ...
    render() {
        let as = this.props.groups.map((el, i) => (
            <a key={i + 1} onClick={this._onClick.bind(this)}>{i + 1}</a>
        ))
        return (
            <div className={styles.container}>
                <div>{as}</div>
                <textarea contentEditable={true} spellCheck={false} onKeyDown={this._onKeyDown.bind(this)} ref={r => this.textarea = r}>{} < /textarea>
                <button type="button" onClick={this._onSubmit.bind(this)}>提交</button>
            </div>
        )
    }
}
